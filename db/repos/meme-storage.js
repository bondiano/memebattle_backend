...
class MemeStorageRepository {
    ...
    get(from, count) {
        return this.db.any('SELECT id, image_src FROM meme_storage WHERE id > $1 LIMIT $2', [from, count]); // TODO: change storage
    }

    count() {
        return this.db.one('SELECT count(*) FROM meme_storage');
    }
}
